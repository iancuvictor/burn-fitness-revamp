...
function Contact() {
  ...
  const searchGym = (word) => {
    ...
    let newArr = arr.filter((locatie) => locatie.locatie.startsWith(word) || locatie.adresa.split(' ').some((cuvant) => cuvant.startsWith(word)));
    setResults(newArr);
    console.log(results);
  };

  const checkSearch = (word) => {
    let result = results.some((item) => item.locatie.includes(word)) || searchWord === ''
    return result;
  }
    ...
    <>
      <div className="flex flex-col items-center pl-[20px] pr-[20px] border-box">
        ...
        <div className="flex flex-col gap-10 justify-center items-baseline md:flex-row md:flex-wrap">
          <div className={checkSearch('zorilor') ? 'block' : 'hidden'} id="zorilor">
            <BlockContact
              locatie="Zorilor"
              nrTel="0771 511 431"
          ...
          <div className={checkSearch('manastur') ? 'block' : 'hidden'} id="manastur">
            <BlockContact
              locatie="Mănăștur"
              nrTel="0771 262 348"
          ...
          <div className={checkSearch('marasti') ? 'block' : 'hidden'} id="marasti">
            <BlockContact
              locatie="Mărăști"
              nrTel="0770 886 179"
          ...
          <div className={checkSearch('flora') ? 'block' : 'hidden'} id="flora">
            <BlockContact
              locatie="Flora"
              nrTel="0774 519 047"
          ...
          <div className={checkSearch('sigma') ? 'block' : 'hidden'} id="sigma">
            <BlockContact
              locatie="Sigma"
              nrTel="0772 269 959"
