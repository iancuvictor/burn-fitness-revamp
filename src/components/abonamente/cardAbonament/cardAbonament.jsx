function CardAbonament(props) {
  ...
  if (props.tier === undefined) {
    ...
    return (
      <div className="h-[480px] w-xs rounded-xl overflow-hidden cursor-pointer relative font-finlandica flex flex-col justify-between pb-[30px] gap-10 items-center bg-black text-white">
        <div className={`z-1 absolute top-0 w-full h-full md:bg-black md:opacity-40 hover:opacity-0 duration-150`}></div>
        ...
        <div className='flex flex-col justify-between items-center h-40 gap-3'>
          ...
          <p className='text-center text-[#B3B3C7] text-[16px]'>{props.desc}</p>
        </div>
      </div>
    );
  }
