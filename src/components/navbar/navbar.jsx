...
function Navbar(){
    ...
    <img src={BurnLogo} alt="burn fitness logo" className='w-40 m-[10px]'/>
    <button 
    onClick={() => setMenuState(!menuState)}
    className={menuState ? 'hidden' : 'md:hidden cursor-pointer text-white text-[25px] pl-[px] pr-[20px]'}
    ><FontAwesomeIcon icon={faBars} /></button>
    <div id='mobileBurger' className={menuState ? 'animate-fade-in flex flex-col fixed justify-center items-center gap-5 w-full h-full bg-black font-finlandica font-bold text-white text-[25px] md:hidden' : 'animate-fade-out hidden'}>
