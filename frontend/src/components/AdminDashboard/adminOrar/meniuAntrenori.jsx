...
export default function MeniuAntrenori() {
    ...
    return <div className='font-finlandica'>
        <div>
            <button onClick={() => {
                ...
                className='cursor-pointer p-3 bg-white shadow-md/20 rounded-md'>Gestionează antrenorii</button>
        </div>
        ...
        z-2 fixed top-0 left-0 h-full w-full bg-black/80`}>
            <div className='bg-white p-5 rounded-md flex flex-col gap-5 overflow-y-scroll'>
                <span className='font-[600]'>Lista antrenori</span>
                <div className='flex flex-row gap-5'>
                    <div className='flex flex-col gap-2'>

                        <span>Introdu numele antrenorului</span>
                        <input onChange={(e) => setData({ ...data, nume: e.target.value })}
                            className='border p-2'
                            type="text" name="" id="" value={data.nume} />
                        <button onClick={() => createInstance()} className='cursor-pointer p-3 bg-rose-500 shadow-md/20 rounded-xs'>Adaugă antrenor</button>
                            </div>
                        <div className='flex flex-col gap-1 h-100 overflow-y-scroll'>
                            {selectors.antrenori.map((item, index) => {
                                return <div key={index} className='flex gap-1'>
                                    <input onChange={() => setData()} type="text" name="" id="" value={item.numeAntrenor} className={inputStyle} />
