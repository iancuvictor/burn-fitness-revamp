...
function Navbar({menuState, setMenuState}){
    ...
    const buttonClass = ({isActive}) => `${isActive ? 'z-1 text-white underline underline-offset-5' : 'hover:text-white hover:underline underline-offset-5 duration-150'} w-50 flex items-center justify-center gap-2`

    return <>
    <div id='navbar' className={`${isAdmin === false ? 'md:flex' : 'hidden'} bg-black/95 z-4 sticky top-0 w-full hidden h-20 gap-5 items-center md:justify-between xl:justify-evenly font-[600]`}>
    ...
    <div id='mobileBurger' className={menuState ? 'z-3 animate-fade-in flex flex-col fixed top-0 justify-center items-center gap-5 w-full h-full bg-black font-finlandica font-bold text-gray-500 text-[25px] md:hidden' : 'animate-fade-out hidden'}>
        ...
        <NavLink to="/profile" className={buttonClass} onClick={() => setMenuState(!menuState)}>Cont</NavLink>
        <div>
